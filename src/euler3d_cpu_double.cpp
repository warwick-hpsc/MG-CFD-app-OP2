...
int main(int argc, char** argv)
{
    ...
    for (int i=0; i<levels; i++) {
        ...
        for(int j = 0; j < nchains; j++){
            ...
            op_par_loop(initialize_variables_kernel, "initialize_variables_kernel", op_nodes[i],
                        op_arg_dat(p_variables[i][var_index], -1, OP_ID, NVAR, "double", OP_WRITE));
        }
    ...
    while(i < conf.num_cycles)
    {
        ...
        op_par_loop(copy_double_kernel, "copy_double_kernel", op_nodes[level],
                    op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX],     -1, OP_ID, NVAR, "double", OP_READ),
        ...
        for (rkCycle=0; rkCycle<RK; rkCycle++)
        {
            ...
            op_par_loop(compute_flux_edge_kernel, "compute_flux_edge_kernel", op_edges[level],
                        op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX], 0, p_edge_to_nodes[level], NVAR, "double", OP_READ),
                        op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX], 1, p_edge_to_nodes[level], NVAR, "double", OP_READ),
            ...
            for(int i = 0; i < nchains; i++){
                ...
                int var_index = i;
#endif
                op_par_loop(test_write_kernel, "ca_test_write_kernel",op_edges[level],
                ...
                op_par_loop(test_read_kernel, "ca_test_read_kernel",op_edges[level],
            ...
            op_par_loop(test_negate_kernel, "ca_test_negate_kernel",op_edges[level],
            ...
            op_par_loop(compute_bnd_node_flux_kernel, "compute_bnd_node_flux_kernel", op_bnd_nodes[level],
                        ...
                        op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX], 0, p_bnd_node_to_node[level], NVAR, "double", OP_READ),
            ...
            op_par_loop(time_step_kernel, "time_step_kernel", op_nodes[level], 
                        ...
                        op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX],     -1, OP_ID, NVAR, "double", OP_WRITE));
        ...
        if (levels <= 1) {
            ...
            if(mg_dir == MG_UP)
            {
                ...
                op_par_loop(up_pre_kernel, "up_pre_kernel", op_nodes[level-1], 
                            op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX],  0, p_node_to_mg_node[level-1], NVAR, "double", OP_WRITE),
                            op_arg_dat(p_up_scratch[level], 0, p_node_to_mg_node[level-1], 1, "int", OP_WRITE));

                op_par_loop(up_kernel, "up_kernel", op_nodes[level-1], 
                            op_arg_dat(p_variables[level-1][DEFAULT_VARIABLE_INDEX], -1, OP_ID, NVAR, "double", OP_READ), 
                            op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX],    0, p_node_to_mg_node[level-1], NVAR, "double", OP_INC),
                ...
                op_par_loop(up_post_kernel, "up_post_kernel", op_nodes[level], 
                            op_arg_dat(p_variables[level][DEFAULT_VARIABLE_INDEX],  -1, OP_ID, NVAR, "double", OP_INC), 
...
void validate_result(op_dat* correct_dat, op_dat* dat, op_set* set, std::string dat_name){
    if (conf.validate_result) {
        ...
        for (int l=0; l<levels; l++) {
            int bad_val_count = 0;
            op_par_loop(count_bad_vals, "count_bad_vals",set[l],
        ...
        if (!value_check_failed) {
            ...
            for (int l=0; l<levels; l++) {
                ...
                op_par_loop(identify_differences, "identify_differences",set[l],
                            op_arg_dat(dat[l],-1,OP_ID,5,"double",OP_READ),
                            op_arg_dat(correct_dat[l],-1,OP_ID,5,"double",OP_READ),
                            op_arg_dat(variables_difference,-1,OP_ID,5,"double",OP_WRITE));

                int count = 0;
                op_par_loop(count_non_zeros, "count_non_zeros",set[l],
