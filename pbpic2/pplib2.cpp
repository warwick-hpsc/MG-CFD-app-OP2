...
/* check if any particles coming from above belong here */
   ...
   if (jsr[0] != 0 && debug)
      fprintf(fp,"%d,Info: particles returning up\n",ks+1);
/* check if any particles coming from below belong here */
   ...
   if (jss[1] != 0 && debug)
      fprintf(fp,"%d,Info: particles returning down\n",ks+1);
...
L180: nps = mpp + jsl[1] + jsr[1] - jss[0];
   ...
   if (ierr > 0) {
	  if(debug){
        fprintf(fp,"Error: particle overflow error, ierr = %d\n",ierr);
	  }
      info[0] = ierr;
      *npp = mpp;
      return;
...
/* check if any particles have to be passed further */
   ...
   if (ibflg[1] > 0) {
	  if(debug){
		fprintf(fp,"Info: particles being passed further = %d\n",
             ibflg[1]);
	  }
      if (iter < itermax)
         goto L60;
      ierr = -((iter-2)/2);
	  if(debug){
		fprintf(fp,"Error: Iteration overflow, iter = %d\n",ierr);
	  }
      info[0] = ierr;
      *npp = mpp;
      return;
...
/* check if buffer overflowed and more particles remain to be checked */
   ...
   if (nter > 0 && debug) {
      fprintf(fp,"Info: %d buffer overflows, nbmax=%d\n",nter,nbmax);
   }
   *npp = mpp;
   return;
}
...
/*--------------------------------------------------------------------*/
void cpptpose_(std::complex<float> *f, std::complex<float> *g, 
			   std::complex<float> *s, std::complex<float> *t, 
			   int *nx, int *ny, int *kxp, int *kyp,
...
/*--------------------------------------------------------------------*/
void cppntpose_(std::complex<float> *f, std::complex<float> *g, 
				std::complex<float> *s, std::complex<float> *t, 
				int *nx, int *ny, int *kxp, int *kyp,
...
void cppmove2_(float *part, float *edges, int *npp, float *sbufr,
               ...
               int *idps, int *nbmax, int *ntmax, int *info, FILE *fp, 
			   bool debug) {
