...
__constant__ int direct_down_v2_kernel_post_stride_OP2CONSTANT;
...
__global__ void op_cuda_down_v2_kernel_post(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    down_v2_kernel_post_gpu(arg0+n,
                        arg1+n*1,
                        arg2+n,
                        arg3+n);
  }
...
void op_par_loop_down_v2_kernel_post(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[21].count==1) || (direct_down_v2_kernel_post_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_down_v2_kernel_post_stride_OP2CONSTANT,&direct_down_v2_kernel_post_stride_OP2HOST,sizeof(int));
    }
