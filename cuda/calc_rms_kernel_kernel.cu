...
__constant__ int direct_calc_rms_kernel_stride_OP2CONSTANT;
int direct_calc_rms_kernel_stride_OP2HOST=-1;
#include "utils.h"

//user function
__device__ void calc_rms_kernel_gpu( 
...
__global__ void op_cuda_calc_rms_kernel(
...
void op_par_loop_calc_rms_kernel(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[14].count==1) || (direct_calc_rms_kernel_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_calc_rms_kernel_stride_OP2CONSTANT,&direct_calc_rms_kernel_stride_OP2HOST,sizeof(int));
    }
    ...
    op_cuda_calc_rms_kernel<<<nblocks,nthread,nshared>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      set->size );
