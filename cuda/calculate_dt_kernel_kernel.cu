...
__constant__ int direct_calculate_dt_kernel_stride_OP2CONSTANT;
...
__global__ void op_cuda_calculate_dt_kernel(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    calculate_dt_kernel_gpu(arg0+n,
                        arg1+n*1,
                        arg2+n*1);
  }
}
...
void op_par_loop_calculate_dt_kernel(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[6].count==1) || (direct_calculate_dt_kernel_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_calculate_dt_kernel_stride_OP2CONSTANT,&direct_calculate_dt_kernel_stride_OP2HOST,sizeof(int));
    }
