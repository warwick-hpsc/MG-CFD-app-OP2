...
__constant__ int direct_compute_step_factor_kernel_stride_OP2CONSTANT;
...
__global__ void op_cuda_compute_step_factor_kernel(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    compute_step_factor_kernel_gpu(arg0+n,
                               arg1+n*1,
                               arg2,
                               arg3+n*1);
  }
...
void op_par_loop_compute_step_factor_kernel(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[8].count==1) || (direct_compute_step_factor_kernel_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_compute_step_factor_kernel_stride_OP2CONSTANT,&direct_compute_step_factor_kernel_stride_OP2HOST,sizeof(int));
    }
