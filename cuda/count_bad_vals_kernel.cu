...
__constant__ int direct_count_bad_vals_stride_OP2CONSTANT;
...
__global__ void op_cuda_count_bad_vals(
...
void op_par_loop_count_bad_vals(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[15].count==1) || (direct_count_bad_vals_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_count_bad_vals_stride_OP2CONSTANT,&direct_count_bad_vals_stride_OP2HOST,sizeof(int));
    }
    ...
    op_cuda_count_bad_vals<<<nblocks,nthread,nshared>>>(
      (double *) arg0.data_d,
      (int *) arg1.data_d,
      set->size );
