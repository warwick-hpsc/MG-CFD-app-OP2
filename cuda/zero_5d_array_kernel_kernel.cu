...
__constant__ int direct_zero_5d_array_kernel_stride_OP2CONSTANT;
...
__global__ void op_cuda_zero_5d_array_kernel(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    zero_5d_array_kernel_gpu(arg0+n);
  }
}
...
void op_par_loop_zero_5d_array_kernel(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[1].count==1) || (direct_zero_5d_array_kernel_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_zero_5d_array_kernel_stride_OP2CONSTANT,&direct_zero_5d_array_kernel_stride_OP2HOST,sizeof(int));
    }
