...
__constant__ int direct_count_non_zeros_stride_OP2CONSTANT;
int direct_count_non_zeros_stride_OP2HOST=-1;
#include "utils.h"

//user function
__device__ void count_non_zeros_gpu( 
    const double* value,
    int* count) {
...
__global__ void op_cuda_count_non_zeros(
...
void op_par_loop_count_non_zeros(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[24].count==1) || (direct_count_non_zeros_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_count_non_zeros_stride_OP2CONSTANT,&direct_count_non_zeros_stride_OP2HOST,sizeof(int));
    }
    ...
    op_cuda_count_non_zeros<<<nblocks,nthread,nshared>>>(
      (double *) arg0.data_d,
      (int *) arg1.data_d,
      set->size );
