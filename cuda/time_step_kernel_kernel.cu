...
__constant__ int direct_time_step_kernel_stride_OP2CONSTANT;
...
void op_par_loop_time_step_kernel(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[11].count==1) || (direct_time_step_kernel_stride_OP2HOST != getSetSizeFromOpArg(&arg2))) {
      ...
      cudaMemcpyToSymbol(direct_time_step_kernel_stride_OP2CONSTANT,&direct_time_step_kernel_stride_OP2HOST,sizeof(int));
    }
