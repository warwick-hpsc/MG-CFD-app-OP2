...
if not os.path.isfile(papidata_filepath):
	...
	if "OFFCORE_RESPONSE_0:ANY_DATA:ANY_RESPONSE" in flux_papi_df["PAPI counter"].values:
		...
		flux_gb_df["actual GB read"] = flux_gb_df["count"]*64/1e9
...
if "flux_iters" in op2_data.columns.values:
	...
	op2_data["effective edges GB read"]  = ((op2_data["nedges"]*3  )/1e9*8).round(5)
	op2_data["effective nodes GB read"]  = ((op2_data["nnodes"]*5*2)/1e9*8).round(5)
	op2_data["effective nodes GB write"] = ((op2_data["nnodes"]*5  )/1e9*8).round(5)
	op2_data["effective GB read"] = op2_data["effective edges GB read"] + op2_data["effective nodes GB read"]
	op2_data["effective GB write"] = op2_data["effective nodes GB write"]
	...
	op2_data_grp_sum = op2_data_grp.sum()[["loop", "nranks", "effective GB"]]
	...
	gb_sec_data.drop(columns=["effective GB", "loop time"], inplace=True)
	bw_df = gb_sec_data

	if "actual GB read" in op2_data.columns.values:
		## PAPI counter for recording actual DRAM traffice can only capture reads, not writes.
		...
		##
		## For each node, 10 doubles are read and 5 written, so can calculate 'actual nodes GB write':
		op2_data["actual nodes GB read"] = op2_data["actual GB read"] - op2_data["effective edges GB read"]
		op2_data["actual nodes GB write"] = op2_data["actual nodes GB read"] / 2.0
		op2_data["actual GB"] = op2_data["effective edges GB read"] + op2_data["actual nodes GB read"] + op2_data["actual nodes GB write"]

		group_cols = [c for c in ["loop", "nranks"] if c in op2_data.columns.values]
		op2_data_grp = op2_data.groupby(group_cols, as_index=False)
		op2_data_grp_mean = op2_data_grp.mean()[["loop", "nranks", "loop time"]]
		op2_data_grp_sum = op2_data_grp.sum()[["loop", "nranks", "actual GB"]]
		gb_sec_data = op2_data_grp_mean.merge(op2_data_grp_sum, validate="one_to_one")
		gb_sec_data["actual GB/sec"] = (gb_sec_data["actual GB"] / gb_sec_data["loop time"]).round(1)
		gb_sec_data.drop(columns=["actual GB", "loop time"], inplace=True)
		bw_df = bw_df.merge(gb_sec_data, validate="one_to_one")

	op2_data.drop(columns=["flux_iters", "nedges", "nnodes"], inplace=True, errors='ignore')
	op2_data.drop(columns=["effective edges GB read", "effective nodes GB read", "effective nodes GB write"], inplace=True, errors='ignore')
	op2_data.drop(columns=["effective GB read", "effective GB write", "effective GB"], inplace=True, errors='ignore')
	op2_data.drop(columns=["actual nodes GB read", "actual nodes GB write"], inplace=True, errors='ignore')
	op2_data.drop(columns=["actual GB read", "actual GB"], inplace=True, errors='ignore')
...
flux_with_ustream_data = flux_data.rename(index=str, columns={"total time":"flux time"})[["nranks", "rank", "flux time"]]
flux_with_ustream_data = flux_with_ustream_data.merge(ustream_data.rename(index=str, columns={"total time":"ustream time"})[["nranks", "rank", "ustream time"]])
...
if "nranks" in op2_data.columns:
	...
	if ustream_data.shape[0] > 0:
		...
		plt.plot(flux_with_ustream_data_grp.mean()[x], flux_with_ustream_data_grp.mean()[y], 'o-', label="compute_flux() pct. ustream()")
		plt.legend()
